import {Component, EventEmitter, Input, OnInit, Output ,OnChanges, SimpleChanges} from '@angular/core';
import { ReactiveFormsModule, FormBuilder, FormGroup, Validators } from '@angular/forms';
import { InputTextModule } from 'primeng/inputtext';
import { ButtonModule } from 'primeng/button';
import {NgClass, NgIf, NgStyle} from '@angular/common';
import {
  PrograssSpinnerComponent
} from '../../../../../../../../../../components/prograss-spinner/prograss-spinner.component';
import {MessageService} from 'primeng/api';
import {ApplicationUserRoleService} from '../../../../../../../../../../services/user-role/application-user-role.service';
import {RequestApplicationRoleDTO} from '../../../../../../../../../../dto/request-application-role-dto';

@Component({
  selector: 'app-update-role',
  imports: [
    ReactiveFormsModule,
    InputTextModule,
    ButtonModule,
    NgIf,
    PrograssSpinnerComponent,
    NgStyle,
    // NgClass
  ],
  templateUrl: './update-role.component.html',
  styleUrl: './update-role.component.scss',
  standalone:true
})
export class UpdateRoleComponent implements OnInit , OnChanges{
  @Input() selectedRole:any;
  @Output() updateRole = new EventEmitter<void>();

  roleForm: FormGroup;
  loading = false
  originalRoleName: string = '';

  constructor(
    private fb: FormBuilder,
    private messageService:MessageService,
    private applicationUserRoleService:ApplicationUserRoleService
  ) {
    this.roleForm = this.fb.group({
      roleName: ['', Validators.required]
    });
  }

  onSubmit() {
    if (this.roleForm.valid) {
      this.loading = true;
      const dto :RequestApplicationRoleDTO  = {
        roleName : this.roleForm.get('roleName')?.value,
      }
      this.applicationUserRoleService.updateRole(this.selectedRole.propertyId,dto)
        .subscribe({
          next: response => {
            this.loading = false;
            this.roleForm.reset();
            this.updateRole.emit();
            this.messageService.add({ severity: 'success', summary: 'Success', detail: response?.message });
          },
          error: error  => {
            this.loading = false;
            console.log('error',error)
            this.messageService.add({ severity: 'warn', summary: 'Warn', detail: error?.error?.message });
          }
        });

    } else {
      this.loading = false;
      this.roleForm.markAllAsTouched();
    }
  }

  resetForm() {
    this.roleForm.reset();
    this.setValues();
  }

  ngOnInit(): void {
  }

  ngOnChanges(changes: SimpleChanges): void {
    if (changes['selectedRole'] && changes['selectedRole'].currentValue) {
      // console.log('selected role is (onchnage)', this.selectedRole);
     this.setValues();
    }
  }

  setValues() {
    this.originalRoleName = this.selectedRole?.role || '';
    this.roleForm.patchValue({
      roleName: this.originalRoleName
    });
  }

  isFormChanged(): boolean {
    return this.roleForm.get('roleName')?.value !== this.originalRoleName;
  }
}
