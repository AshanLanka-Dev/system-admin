...
@Component({
  selector: 'app-online-vet-care',
  imports: [
    AdminHeaderComponent,
    BreakCrumbForLmsComponent,
    TabViewModule, CommonModule, RouterOutlet
  ],
  templateUrl: './online-vet-care.component.html',
  styleUrl: './online-vet-care.component.scss',
  standalone:true
})
export class OnlineVetCareComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.router.events.pipe(
      ...
      if (currentUrl.includes('/request-list')) {
  ...
  handleTabChange(event: any) {
    // Navigate to the appropriate route based on the selected tab
     ...
     if (event.index === 0) {
      this.router.navigate(['/dashboard/process/admin/online-vet-care/pending-request-list']);
    }
    else if (event.index === 1) {
      this.router.navigate(['/dashboard/process/admin/online-vet-care/pet-profile']);
    }
    else if (event.index === 2) {
      this.router.navigate(['/dashboard/process/admin/online-vet-care/request-list']);
    }
  }
}
